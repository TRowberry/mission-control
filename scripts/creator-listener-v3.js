/**
 * Creator Agent Listener v3 - Mission Control Only
...
const MC_API_KEY = 'mc_agent_6a58a4c24ad658486edc445d58c28a9d';
...
// Ask Ollama
async function askCreator(prompt) {
  const systemPrompt = `You are Creator, a creative content assistant for Tends2Trend. You help with:
- Video content ideas and scripts
- Caption writing for TikTok, Instagram, and YouTube
- Analyzing trending content formats
- Suggesting hooks, angles, and creative approaches
- Brand voice and tone guidance

Be creative, enthusiastic, and practical. Focus on viral-worthy content that engages audiences.`;
...
async function processMCMessage(msg) {
  ...
  // Check if message mentions creator
  if (!/\bcreator\b/i.test(body)) return;
  ...
  if (!request) { 
    await sendMCMessage(channelId, 'Hey! Need creative content ideas? 🎨'); 
    return; 
  }
  ...
  try {
    ...
    const response = await askCreator(request);
...
// Startup
console.log('🎨 Creator Agent v3 - Mission Control');
console.log('======================================');
console.log(`MC URL: ${MC_URL}`);
console.log(`Ollama: ${OLLAMA_URL} (${MODEL})`);
console.log(`Poll interval: ${POLL_INTERVAL/1000}s`);
console.log('Listening for mentions of "creator"...\n');
